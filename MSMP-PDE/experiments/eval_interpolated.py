...
def compute_L2_norms_U(model: torch.nn.Module,
                         batch_size: int,
                         nr_gt_steps: int,
                         loader: DataLoader,
                         loader_U: DataLoader,
    ...
    losses = None
    norms = None
    for ((u_base, u_super, x, variables),(u_base_u, u_super_u, x_u, variables_u)) in zip(loader,loader_U):
        ...
        with torch.no_grad():
            ...
            data_u, labels_u = graph_creator.create_data(u_super_u,same_steps)
            if f'{model}' == 'GNN':
                ...
                data_u, labels_u = data.to(device), labels.to(device)
                ...
                if hasattr(model,'eq_variables'):
                    ...
                    pred = model(data)
                new_pred = torch.empty_like(pred).cuda()
                for b, batch_data in enumerate(pred):
                    for t,time_data in enumerate(batch_data):
                        for d,dim_data in enumerate(time_data):
                            #print("interp")
                            f = interpolate.interp1d(x[0,:].cpu().numpy(),dim_data.cpu().numpy())
                            new_pred[b,t,d,:] = torch.tensor(f(x_u[0,:])).cuda()
                loss = torch.square(new_pred - labels)
...
def main(args: argparse):
    ...
    # Load datasets
    train_string = f'data/{pde}_train_{args.experiment}_I.h5'
    valid_string = f'data/{pde}_valid_{args.experiment}_I.h5'
    test_string = f'data/{pde}_test_{args.experiment}_I.h5'
    try:
    ...
    try:
        train_dataset_U = HDF5Dataset(train_string, pde=pde, mode='train', base_resolution=base_resolution, super_resolution=super_resolution)
        train_loader_U = DataLoader(train_dataset_U,
                                  batch_size=args.batch_size,
                                  shuffle=True)

        valid_dataset_U = HDF5Dataset(valid_string, pde=pde, mode='valid', base_resolution=base_resolution, super_resolution=super_resolution)
        valid_loader_U = DataLoader(valid_dataset_U,
                                  batch_size=args.batch_size,
                                  shuffle=False)

        test_dataset_U = HDF5Dataset(test_string, pde=pde, mode='test', base_resolution=base_resolution, super_resolution=super_resolution)
        test_loader_U = DataLoader(test_dataset_U,
                                 batch_size=args.batch_size,
                                 shuffle=False)
    except:
        raise Exception("Datasets could not be loaded properly")
    #print("_____________________________")
    #print(np.sum(np.square(test_dataset.data["pde_250-100"][0] - test_dataset_U.data["pde_250-100"][0])))
    ...
    if args.model_to_test is not None:
        ...
        print("******** Norm func ********")
        losses2, losses_rel = compute_L2_norms_U(model=model,
                                    batch_size=args.batch_size,
                                    nr_gt_steps=args.nr_gt_steps,
                                    loader=test_loader,
                                    loader_U=test_loader_U,
